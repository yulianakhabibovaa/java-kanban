...
class FileBackedTaskManagerTest extends TaskManagerTest<FileBackedTaskManager> {
    ...
    void shouldSaveTasksToFileAndLoadFromIt() throws IOException {
        ...
        assertEquals("0,TASK,задача,NEW,я задача,30," + now.format(Task.DATE_TIME_FORMATER) + ",", lines.get(1));
        assertEquals("1,EPIC,эпик,NEW,я эпик,30," + now.plusMinutes(30L).format(Task.DATE_TIME_FORMATER) + ",", lines.get(2));
        assertEquals("2,SUBTASK,задача,NEW,я подзадача,30," + now.plusMinutes(30L).format(Task.DATE_TIME_FORMATER) + ",1", lines.get(3));
